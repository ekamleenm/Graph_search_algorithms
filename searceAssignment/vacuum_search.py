...
class VacuumPlanning(Problem):
    ...
    def path_cost(self, curNode, state1, action, state2):
        ...
        # Reduce cost when staying on the left side
        if state2[0] < state1[0]:
            cost -= 0.1

        # Reduce cost when staying on the upper side
...
class Gui(VacuumEnvironment):
    ...
    def setupTestEnvironment(self):
        ...
        if self.agent is None:
            ...
            self.add_agent(XYSearchAgent(program=XYSearchAgentProgram, loc=(xi, yi)), (xi, yi))
